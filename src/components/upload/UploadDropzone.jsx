...
/**
 * Komponen Dropzone untuk mengunggah file.
 * @param {object} props
 * @param {function} props.onFileDrop - Fungsi callback yang akan dipanggil saat file dipilih/di-drop.
 * @param {File | null} props.file - Objek file yang sedang aktif.
 */
export default function UploadDropzone({ onFileDrop, file }) {
  const onDrop = useCallback(
    (acceptedFiles) => {
      // Memastikan hanya satu file yang diproses dan memanggil callback dari parent
  ...
  const handleRemoveFile = (e) => {
    e.stopPropagation(); // Mencegah event bubbling ke dropzone
    onFileDrop([]); // Mengirim array kosong untuk mereset file di parent
  };
