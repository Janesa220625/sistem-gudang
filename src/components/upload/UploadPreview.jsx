...
/**
 * Komponen untuk menampilkan data yang sudah diproses dalam bentuk tabel pratinjau.
 * @param {object} props
 * @param {Array<object>} props.data - Array data yang sudah diproses dengan status validasi.
 */
export default function UploadPreview({ data }) {
  ...
  const ProductValidationBadge = ({ status }) => {
    switch (status) {
      case "ok":
        return (
  ...
  const DuplicateValidationBadge = ({ status }) => {
    switch (status) {
      case "unik":
        return (
          <Badge variant="default" className="bg-green-600 hover:bg-green-700">
            ...
            {data.map((row, index) => (
              <TableRow
                ...
                className={`transition-colors ${
                  ...
                  row.validation?.duplicate === "duplikat"
                    ? "bg-red-500/10"
                    : ""
                }`}
              >
                ...
                <TableCell className="text-center">
                  <DuplicateValidationBadge
                    status={row.validation?.duplicate}
                  />
                </TableCell>
              </TableRow>
            ))}
