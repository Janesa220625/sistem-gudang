...
import { BarChart2, Filter, Search, DollarSign } from 'lucide-react';
...
function ProductAnalysis() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const aggregatedData = orders.flatMap(order => 
        ...
        const orderItemTotalQuantity = orders.find(o => o.id === item.order_id).order_items.reduce((sum, i) => sum + i.quantity, 0);
        const itemRevenue = orderItemTotalQuantity > 0 ? (item.quantity / orderItemTotalQuantity) * item.order_total : 0;
      ...
      setStoreAccounts(accountsData || []);
       ...
       toast({ title: 'Error', description: `Gagal memuat data analisis: ${error.message}`, variant: 'destructive' });
    } finally {
        setLoading(false);
    }
  }, [user, toast]);
  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
        ...
        <p className="text-gray-400 text-lg">Pahami performa setiap varian produk di semua marketplace</p>
      </motion.div>
      ...
      >
        ...
        <Card className="glass-effect border-white/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-300">Estimasi Profit</CardTitle>
            <DollarSign className="h-4 w-4 text-green-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-400">Rp {stats.totalProfit.toLocaleString('id-ID', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</div>
          </CardContent>
        </Card>
        <Card className="glass-effect border-white/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-gray-300">Varian Unik Terjual</CardTitle>
            <BarChart2 className="h-4 w-4 text-gray-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats.uniqueProducts}</div>
          </CardContent>
        </Card>
        <Card className="glass-effect border-white/20">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{stats.totalItemsSold.toLocaleString('id-ID')}</div>
          </CardContent>
        </Card>
      </motion.div>
          ...
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="relative lg:col-span-1">
                ...
                <Input placeholder="Cari SKU Varian atau Nama Produk..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="pl-10 glass-effect border-white/30" />
              </div>
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="glass-effect border-white/30"><SelectValue placeholder="Urutkan Berdasarkan" /></SelectTrigger>
                <SelectContent>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
        <Card className="glass-effect border-white/20">
          <CardHeader>
            <CardTitle className="text-white flex items-center gap-2"><BarChart2 className="h-5 w-5" />Peringkat Produk</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-gray-300">SKU Varian / Produk</TableHead>
                    <TableHead className="text-gray-300 text-right">Terjual</TableHead>
                    <TableHead className="text-gray-300 text-right">Pendapatan</TableHead>
                    <TableHead className="text-gray-300 text-right">Estimasi Profit</TableHead>
                    <TableHead className="text-gray-300">Platform</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    <TableRow><TableCell colSpan="6" className="text-center text-gray-400 py-10">Memuat data...</TableCell></TableRow>
                    ...
                    <TableRow><TableCell colSpan="6" className="text-center text-gray-400 py-12">
                      ...
                      <p className="font-semibold">Data produk tidak ditemukan</p>
                      <p className="text-sm text-gray-500">Coba sesuaikan filter atau upload data pesanan terlebih dahulu.</p>
                    </TableCell></TableRow>
                  ) : paginatedData.map((product, index) => (
                    <TableRow key={product.sku}>
                      <TableCell className="text-lg font-bold text-gray-400">{(currentPage - 1) * ITEMS_PER_PAGE + index + 1}</TableCell>
                      <TableCell>
                        <div className="font-medium text-white">{product.productName}</div>
                        <div className="text-sm text-cyan-400">{product.sku}</div>
                      </TableCell>
                      <TableCell className="text-white font-bold text-right">{product.quantity.toLocaleString('id-ID')}</TableCell>
                      <TableCell className="text-white font-medium text-right">Rp {product.revenue.toLocaleString('id-ID', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</TableCell>
                      <TableCell className={`font-bold text-right ${product.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        Rp {product.profit.toLocaleString('id-ID', {minimumFractionDigits: 0, maximumFractionDigits: 0})}
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {product.platforms.map(p => (
                            <span key={p} className={`px-2 py-1 rounded-full text-xs capitalize ${getPlatformColor(p)}`}>{p}</span>
                          ))}
                        </div>
                      </TableCell>
                    </TableRow>
