...
import { Undo2, Info, Upload, Download, FileSpreadsheet } from 'lucide-react';
...
const ReturnedOrders = () => {
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Template Retur');
    XLSX.writeFile(workbook, 'Template_Pesanan_Retur.xlsx');
    toast({ title: 'Template Diunduh', description: 'Isi dengan No. Pesanan yang diretur.' });
  };
    ...
    reader.onload = async (e) => {
      try {
        ...
        toast({
          ...
          description: `${updatedCount} pesanan telah ditandai sebagai 'Retur'. ${orderSnList.length - updatedCount} pesanan tidak ditemukan.`,
        });
        resetState();
      } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
        <h1 className="text-4xl font-bold gradient-text">Update Pesanan Retur</h1>
        <p className="text-gray-400 text-lg">Upload daftar No. Pesanan untuk menandainya sebagai 'Retur'.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            <Card className="glass-effect border-white/20 h-full">
                <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2"><Upload className="h-5 w-5" /> Unggah Laporan Retur</CardTitle>
                </CardHeader>
                <CardContent>
                    {file ? (
                        <div className="space-y-4 text-center">
                            ...
                            <Button onClick={processFile} disabled={processing} className="w-full bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700">
                                <Undo2 className="mr-2 h-4 w-4" />
                                {processing ? 'Memproses...' : 'Update Status Retur'}
                            </Button>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="space-y-8">
            <Card className="glass-effect border-white/20">
                <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2"><Download className="h-5 w-5"/> Template Retur</CardTitle>
                ...
                <CardContent className="flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p className="text-gray-300">Cukup isi kolom 'No. Pesanan' dengan data retur Anda.</p>
             ...
             <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                ...
                <div>
                    ...
                    <p className="text-gray-400">Sistem akan menandai semua No. Pesanan yang valid di dalam file sebagai 'Retur'. Stok produk tidak akan otomatis ditambahkan kembali.</p>
                </div>
            </div>
        </motion.div>
      </div>
